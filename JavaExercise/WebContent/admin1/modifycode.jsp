...
<%
CodeController ff=new CodeController();
Code fa=ff.getCodebyId(Integer.parseInt(request.getParameter("informno")));
%>
   <form id="news" method="post" action="/admin/CompileAndRunJavaFile?cmd=modify" style="margin:20px">
   <input name="uid" style="display:none" value="<%= fa.getCodeid()%>">
	    <p>程序标题：<input  name="title" size="100" style="margin:2px" value="<%= fa.getTitle()%>"/></p>
	    <p>程序描述：<textarea  name="describe"  style="margin:2px;height:50px;width:630px;" /><%=fa.getCdescribe() %></textarea></p>
	    <p>正确代码：<textarea id='code'  name="code"  style="margin:2px;height:150px;width:630px;" /></textarea></p>
	    <p>出题代码：<textarea  name="content" style="margin:2px;height:150px;width:630px;" /><%=fa.getCcontent() %></textarea></p>
